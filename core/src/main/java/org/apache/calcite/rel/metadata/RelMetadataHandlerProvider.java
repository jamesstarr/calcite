...
/**
 * Provides {@link MetadataHandler} call sites and {@link RelMetadataCache} for
 * {@link RelMetadataQuery}. The handlers provided are responsible for
 * updating the cache stored in {@link RelMetadataQuery}.
 */
public interface RelMetadataHandlerProvider {

  <H extends MetadataHandler<M>, M extends Metadata> H initialHandler(MetadataDef<M> handlerClass);

  /** Re-generates the handler for a given kind of metadata.. */
  <H extends MetadataHandler<M>, M extends Metadata> H revise(
      Class<? extends RelNode> relClass, MetadataDef<M> handlerClass);
