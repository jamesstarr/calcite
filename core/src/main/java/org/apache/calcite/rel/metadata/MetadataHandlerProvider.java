...
public interface MetadataHandlerProvider {
  ...
  <H extends MetadataHandler<M>, M extends Metadata> H initialHandler(MetadataDef<M> def);

  /** Re-generates the handler for a given kind of metadata, adding support for
   * {@code class_} if it is not already present. */
  <H extends MetadataHandler<M>, M extends Metadata> H revise(MetadataDef<M> def);
...
// End MetadataHandlerProvider.java
