 ...
 * plugin mechanism.
 */
public class JaninoRelMetadataQuery extends RelMetadataQuery {
  ...
  public JaninoRelMetadataQuery(JaninoRelMetadataProvider metadataProvider) {
  ...
  protected <M extends Metadata, H extends MetadataHandler<M>> H
    ...
    return ((JaninoRelMetadataProvider) metadataProvider).revise(class_, def);
  }
   ...
   */
  @Override public Multimap<Class<? extends RelNode>, RelNode> getNodeTypes(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public Double getMaxRowCount(RelNode rel) {
    for (;;) {
      try {
   ...
   * @return max row count
   */
  @Override public Double getMinRowCount(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public RelOptCost getCumulativeCost(RelNode rel) {
    for (;;) {
      try {
   ...
   * @return estimated cost, or null if no reliable estimate can be determined
   */
  @Override public RelOptCost getNonCumulativeCost(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public Double getPercentageOriginalRows(RelNode rel) {
    for (;;) {
      try {
        Double result =
   ...
   * all)
   */
  @Override public Set<RelColumnOrigin> getColumnOrigins(RelNode rel, int column) {
    for (;;) {
      try {
   ...
   */
  @Override public Set<RexNode> getExpressionLineage(RelNode rel, RexNode expression) {
    for (;;) {
      try {
   ...
   */
  @Override public Set<RelTableRef> getTableReferences(RelNode rel) {
    for (;;) {
      try {
   ...
   * reliable estimate can be determined
   */
  @Override public Double getSelectivity(RelNode rel, RexNode predicate) {
    for (;;) {
      try {
   ...
   */
  @Override public Set<ImmutableBitSet> getUniqueKeys(RelNode rel,
                                            boolean ignoreNulls) {
    for (;;) {
      try {
   ...
   */
  @Override public Boolean areColumnsUnique(RelNode rel, ImmutableBitSet columns,
                                  boolean ignoreNulls) {
    for (;;) {
      try {
   ...
   * null if not enough information is available to make that determination
   */
  @Override public ImmutableList<RelCollation> collations(RelNode rel) {
    for (;;) {
      try {
   ...
   *
   */
  @Override public Double getPopulationSize(RelNode rel,
   ...
   */
  @Override public Double getAverageRowSize(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public List<Double> getAverageColumnSizes(RelNode rel) {
    for (;;) {
      try {
   ...
   * known
   */
  @Override public Boolean isPhaseTransition(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public Integer splitCount(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public Double memory(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public Double cumulativeMemoryWithinPhase(RelNode rel) {
    for (;;) {
      try {
   ...
   */
  @Override public Double cumulativeMemoryWithinPhaseSplit(RelNode rel) {
    for (;;) {
      try {
   ...
   * defaults to true
   */
  @Override public boolean isVisibleInExplain(RelNode rel,
...
// End JaninoRelMetadataQuery.java
